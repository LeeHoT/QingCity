...
//
//        // 设置用户相关数据
//        UserRedis.getInstance().alter(String.valueOf(userID), User.toMap(userEntity));
//
//        // 存储开放平台ID+来源 与 游戏平台ID 对应关系
//        OpenUserRedis.getInstance().add(userEntity.getOpenUserID(), userEntity.getOpenSource(), String.valueOf(userID));
//
//        // 初始化排行榜数据
//        RankRedis.getInstance().add(userEntity.getScore(), String.valueOf(userEntity));
//
//        return userID;
//    }
